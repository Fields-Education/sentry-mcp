...
# Refresh the lockfile, then align this package's resolutions with mcp-cloudflare
...
### Why `align-lockfile.mjs` is required

Because `src/` and `vite.config.ts` are symlinks, this package and
`@sentry/mcp-cloudflare` must resolve the **same** physical dependency
instances. `pnpm` resolves vite's optional peers (`jiti`, `yaml`, ...)
independently per workspace importer, which produces two distinct `vite`
instances and fails the build with:

```
error TS2769: Two different types with this name exist, but they are unrelated.
```

The script copies upstream's resolved versions onto this package's importer for
every dependency that shares the same specifier. It is idempotent, so it is safe
to run after every `pnpm install`.

### Keeping dependencies in sync

`package.json` intentionally mirrors `../mcp-cloudflare/package.json`
(dependencies, devDependencies and `version`). Only `name`, `description`,
`deploy:canary` and `test:ci` differ. When upstream changes dependencies, copy
them over verbatim rather than resolving them independently.
